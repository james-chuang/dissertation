...
In the course of developing a ChIP-Seq pipeline, I came across the following error in a published spike-in normalization method.
Throughout, I use `experimental' and `spike-in' to refer to the two genomes, e.g., experimental signal and spike-in signal.

The goal when including spike-ins in a ChIP-seq experiment is to end up with a normalized experimental signal which is proportional to the absolute abundance of the factor being immunoprecipitated.
...
However, this would not account for differences in $\phi$ between samples.
Two libraries representing the same condition and sequenced to the same depth should have equivalent values of $\frac{R_{\text{spike}}}{C_{\text{spike}}}$ but they would not if they differed in the amount of spike-in added.
...
\end{align*}
From here, it's simple to calculate $\alpha$ by setting this value to be the same for all samples.
Since the actual value of the spike-in signal doesn't matter as long as it's the same for all libraries, we can arbitrarily set it to $1$ for convenience:
\begin{align*}
    \alpha \frac{R_\text{spike} C_\text{exp}}{C_\text{spike}} &= 1 \\
    \alpha &= \frac{C_\text{spike}}{R_\text{spike} C_\text{exp}}.
\end{align*}
Notice that you only need to know the ratio of spike-in to experimental cells to calculate $\alpha$, and not their absolute values.
We can also rewrite this in terms of $\phi$, the proportion of the sample that was spike-in cells:
\begin{align*}
...
\end{align*}
Notice how this form for $\alpha$ is different than the one derived in \citet{orlando}:
\begin{align*}
    \alpha &= \frac{\phi}{R_\text{spike} \left(1-\phi \right)} & \alpha_\text{orlando} &= \frac{\phi}{R_\text{spike}}.
\end{align*}
Let's do some examples with both versions to make sure the $\alpha$ values we get make sense, and compare them to $\alpha_\text{orlando}$.
First, we'll vary sequencing depth, keeping everything else constant.
...
\end{align*}
The total levels of spike-in normalized experimental signal can be found for each library by multiplying $\alpha$ by $R_\text{exp}$:
\begin{align*}
    ...
    \text{signal}_{\text{orlando}_1} &= \frac{1}{20} \left(16\right) & \\
    \text{signal}_1 &=  1 &
    \text{signal}_2 &=  1 &
...
\begin{align*}
    ...
    \text{signal}_{\text{orlando}_1} &= 0.15 &
\end{align*}
...
Notice how $\alpha_\text{input}$ reduces down to normalizing by the experimental library size, with no dependence at all on the spike-in. This makes sense because the input always represents the same state, regardless of how much spike-in is added to it. The function of the spike-in in the input is only to allow us to estimate abundances in the corresponding IP library. Rewriting $\alpha_\text{IP}$ in the form below shows that $\alpha_\text{IP}$ will basically scale the experimental IP signal to the same scale as the experimental input signal, using the spike-in as a link between the two samples. This makes it natural to subtract the normalized input signal from the normalized IP signal: since they are on the same scale, the resulting coverage can be interpreted as reporting how much more IP signal was detected than was expected based on the input.
