...
main = function(theme_spec,
    ...
    ridgelines = ggplot() +
        ...
        scale_fill_viridis(option="viridis",
                           ...
                           breaks = c(quantile(df[["relative_log_enrichment"]], trim_pct, na.rm=TRUE),
                                      quantile(df %>%
                                                   filter(! is.infinite(relative_log_enrichment)) %>%
                                                   pull(relative_log_enrichment),
                                               1-trim_pct, na.rm=TRUE)),
    ...
    gene_diagram = ggplot(data = annotation) +
        ...
        ggtitle("MNase dyads, TFIIB protection, and sense TSSs") +
        theme_void() +
    ...
    figure = plot_grid(gene_diagram, ridgelines,
              align = "v", axis="lr",
              ncol=1,
