...
main = function(theme_spec,
    ...
    df = parse_motifs(antisense_path,
        ...
        bind_rows(parse_motifs(antisense_path,
                               direction="antisense",
                               category="antisense")) %>%
        bind_rows(parse_motifs(intragenic_path,
                               category="intragenic")) %>%
        bind_rows(parse_motifs(intragenic_path,
                               direction="antisense",
                               category="intragenic")) %>%
...
get_motif_metadata = function(meme_path){
    ...
    motif_plots = arrangeGrob(motif_plot(df= df %>%
                                             filter(category=="intragenic"),
                                         df_annotation= get_motif_metadata(intragenic_path),
                                         title="intragenic"),
                              motif_plot(df= df %>%
                                             filter(category=="antisense"),
                                         df_annotation= get_motif_metadata(antisense_path),
                                         title="antisense"),
                              heights=c(0.32, 0.68))
