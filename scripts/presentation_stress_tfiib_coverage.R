...
main = function(theme_spec,
    ...
    annotation = import_bed(transcript_annotation) %>%
        ...
        filter(gene_id %in% c("ASE1", "DSK2", "MRP20"))
    ...
    gene_diagrams = ggplot(data = annotation) +
        ...
        geom_text(aes(x=(orf_start+orf_end)/2,
                  ...
                  y=0, size=11/72*25.4, parse=TRUE,
                  family="FreeSans") +
    ...
    df %<>%
        filter(group %in% c("untreated",
                            "diamide"),
               gene %in% c("ASE1", "DSK2", "MRP20")) %>%
        distinct() %>%
    ...
    figure = plot_grid(gene_diagrams, coverage_plot,
              align = "v", axis="lr",
              ncol=1,
