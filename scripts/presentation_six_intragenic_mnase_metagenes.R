...
import = function(path,
    ...
    df = read_tsv(path,
        ...
        group_by(annotation) %>%
        mutate(anno_labeled = paste0(if_else(str_detect(annotation, "cluster"),
                                             str_remove(annotation, "intragenic "),
                                             annotation),
                                     if_else(str_detect(annotation, "cluster"), " (", "\n("),
                                     n_distinct(index),
                                     " TSSs)"))
    if (normalize){
        df %<>%
...
metagene = function(df,
    ...
    plot = ggplot() +
        ...
        scale_x_continuous(expand = c(0,0),
                           ...
                           labels = function(x)case_when(x==0 ~ "TSS",
                                                         x==0.4 ~ paste(x, "kb"),
                                                         TRUE ~ as.character(x))) +
        scale_y_continuous(breaks = scales::pretty_breaks(n=if_else(assay=="GC%", 3, 2)),
                           name = ylabel) +
        ggtitle(assay) +
        theme_default_presentation +
        theme(legend.justification = c(0.5, 0.5),
              # legend.background = element_blank(),
              legend.position = c(0.158, 0.837),
              legend.margin= margin(16.5,21,16.5,21,"pt"),
    ...
    if (assay != "GC%"){
        plot = plot +
            geom_label(data = df %>%
                           distinct(anno_labeled, .keep_all=TRUE),
...
main = function(theme_spec = "thesis_theme.R",
    ...
    mnase_df = import(mnase_data) %>%
        mutate(column = case_when(index==1 ~ 1,
                                  index==5 ~ 3,
                                  TRUE ~ 2),
               row = case_when(index==2 ~ 1,
                               index==4 ~ 3,
                               TRUE ~ 2))
