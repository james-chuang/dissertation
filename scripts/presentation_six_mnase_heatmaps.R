...
main = function(theme_spec,
    ...
    netseq_df = read_tsv(netseq_data,
        ...
        ungroup() %>%
        mutate(group = "phantom(log[2](p/T)) ~ \"wild-type\" ~ phantom(log[2](p/T))")
    ...
    mnase_df = read_tsv(mnase_data,
        ...
        mutate(group = ordered(group,
                               ...
                               labels = c("phantom(g[2](p/T)) ~ \"wild-type\" ~ phantom(g[2](p/T))",
    ...
    figure = plot_grid(netseq_plot, mnase_plot, align="h", axis="tb", nrow=1,
                     rel_widths = c(0.17, 1))
